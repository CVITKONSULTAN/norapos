...
@section( 'page_name', "Marketing Campagins - Create new email message")
...
@section('page_css')
    ...
    <style>
        ...
        .contact_data_modals > .content {
            padding: 0px !important;
        }
        .contact_data_modals > .content > .list_item {
            padding-left: 0px;
            height: 400px;
            overflow: auto;
        }
...
@section('content-header',"Create new email message")
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    ...
                    <form id="useTemplate" >
                        @csrf
                        <input type="hidden" name="template_email_id" value="0" />
                        ...
                        <div class="row cstm_preview">
                            <div class="col-md-9">
                                ...
                                <div class="form-group">
                                    <textarea rows="5" name="email_description" class="form-control" placeholder="Email link description"></textarea>
                                </div>
                            </div>
                            <div class="col-md-3">
        ...
        <div class="header">
            <div class="form-group">
                <div class="input-group">
                    ...
                    <input id="searching_cdf" class="form-control" placeholder="Search ..." />
                </div>
            </div>
        </div>
        <div class="content">
            <p class="header_title">Contact data fields</p>
            <ul class="list_item" id="contact_df"></ul>
        </div>
    </div>
            ...
            <div class="modal-content">
                <div class="modal-body">
                    ...
                    <div class="header_page">
                        Preview Template
                        <a href="{{ route("marketing.newsletter.pagebuilder") }}" class="btn btn-primary btn-sm">Load in builder</a>
                    </div>
                    <div class="embed-responsive embed-responsive-4by3 preview_template">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <select id="from_select2" class="form-control">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label>Reply-to Email Adress:</label>
                                <select id="to_select2" class="form-control">
                        ...
                        <div class="col-md-12">
                                ...
                                >
                                    <label class="label_emoji">Subject Line:</label>
                                    <div class="emoji-button" id="emoji_picker_subject">&#x1f604;</div>
                                    <div class="contact_data_button">&#128101;</div>
                            ...
                            <div class="list_screen">
                                <div class="list_action">
                                    ...
                                    <div class="select_category">
                                        ...
                                        <a href="{{ route('marketing.newsletter.pagebuilder') }}" class="btn btn-primary">Build from scratch</a>
                                    </div>
...
@section('javascript')
    ...
    <script>
        ...
        // front end for modals contact data field
        let status_modal_contact = false;
        $(".contact_data_button").click(function(e){
            !status_modal_contact ? $(".contact_data_modals").show() : $(".contact_data_modals").hide();
            status_modal_contact = !status_modal_contact;
        });
        $(document).mouseup(function(e) {
            var container = $(".contact_data_modals");
            var container2 = $(".contact_data_button");

            // if the target of the click isn't the container nor a descendant of the container
            if (!container.is(e.target) && container.has(e.target).length === 0 && !container2.is(e.target) && container2.has(e.target).length === 0) 
            {
                status_modal_contact = false;
                container.hide();
            }
        });

        // render for item in contact data field
        let item_contact = [
            {
                id:1,
                label:"birthdate",
        ...
        function renderDataField(data) {
            ...
            data.map((item,index)=>{
                let string = '<li>'+
                    '<a class="item_data" href="javascript:void(0)" onClick="insertCodetoField('+"'"+item.code+"'"+')"><span class="item_label">'+item.label+'</span> <span class="item_code">'+item.code+'</span></a>'+
                '</li>';
                $("#contact_df").append(string);
            });
        }

        function insertCodetoField(code) {
            var cursorPos = $('#input1').prop('selectionStart');
            var v = $('#input1').val();
            var textBefore = v.substring(0,  cursorPos);
            var textAfter  = v.substring(cursorPos, v.length);

            $('#input1').val(textBefore + code + textAfter);

            $(".contact_data_modals").hide();
            status_modal_contact = false;
        }

        $(document).ready((e)=>{
            renderDataField(item_contact);
        });

        $("#searching_cdf").keyup(function(e){
            let val = $(this).val();
            let data = [];
            if(val.length > 0) {
                item_contact.map((item,index)=>{
                    if( item.label.includes(val) ) data.push(item);
                });
                renderDataField(data);
            } else {
                renderDataField(item_contact);
            }
        });
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $('#from_select2').select2({
                ajax: {
                    ...
                    processResults: function (response) {
                        ...
                        let from_email_list = response.data.from_email_list;
                        if(from_email_list == null) return [];

                        let data = from_email_list.map((item,index)=>{
                            return {
                                "id":item.id,
                                "text":item.email,
                                "value":item
            ...
            $('#from_select2').on('select2:select', function (e) {
            ...
            $('#to_select2').select2({
                ajax: {
                    ...
                    processResults: function (response) {
                        ...
                        response.data = JSON.parse(response.data);
                        let reply_email_list = response.data.reply_email_list;
                        if(reply_email_list == null) return [];

                        let data = reply_email_list.map((item,index)=>{
                            return {
                                "id":item.id,
                                "text":item.email,
                                "value":item
            ...
            $('#to_select2').on('select2:select', function (e) {
                var data = e.params.data;
                to = data.value;
            });
...
<script>
    ...
    $("#category_form").validate({
        submitHandler:function(form){
            ...
            input.template_email_category_list = [gotForm];
            console.log(input);
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                if(response.status){
                    ...
                    let email_news_letter_template_list = data.email_news_letter_template_list;

                    elm.empty();

                    if(email_news_letter_template_list !== null){

                        email_news_letter_template_list.map((item,index)=>{
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                if(response.status){
                    ...
                    let data = JSON.parse(response.data);
                    data = data.template_email_preview[0];

                    $("#useTemplate input[name=template_email_id]").val(data.template_id);
                    $("#useTemplate input[name=email_message_name]").val(data.template_name);
                    $("#useTemplate input[name=email_message_web_link_name]").val(data.template_id);
                    $("#useTemplate textarea[name=email_description]").val(data.template_description);
                ...
                $.ajax({
                    ...
                    success:function(response){
                        ...
                        if(response.status){
                            ...
                            let data = JSON.parse(response.data);
                            console.log(data);
                            let url = "{{ route('marketing.newsletter.pagebuilder') }}/"+data.item_id+"/build";
                            location.href = url;
