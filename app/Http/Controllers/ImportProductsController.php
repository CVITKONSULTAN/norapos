...
class ImportProductsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // try {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    if (!empty($sku)) {
                        ...
                        if ($is_exist) {
                            ...
                            $error_rows[] = ["message"=>$error_msg,"data"=>$value];
                            // break;
                        }
                    } else {
                        $product_array['sku'] = ' ';
                        ...
                        if (!empty($value[20]) && $enable_stock == 1) {
                            ...
                            if (!empty($value[22])) {
                                try{
                                    ...
                                    $error_rows[] = ["message"=>$e->getMessage(),"data"=>$value];
                                }
                            } else {
                        ...
                        if ($product->type == 'single') {
                            ...
                            if (!empty($opening_stock)) {
                                try{
                                    ...
                                    DD($e->getMessage(),$value,$opening_stock);
                                }
                            }
            ...
            if(count($error_rows)){
                $output = [
                        'success' => 0,
                        'msg' => "Ada baris data yang tidak sesuai format",
                        "data"=>$error_rows
                ];
                DD($output);
    ...
    private function addOpeningStock($opening_stock, $product, $business_id)
    {
        ...
        // try{
            $qty = is_numeric($opening_stock['quantity']) ? $opening_stock['quantity'] : 0;
            $total_before_tax = $qty * $variation->dpp_inc_tax;
        // }catch(\Exception $e){
        //     DD($opening_stock,$variation);
        // }
